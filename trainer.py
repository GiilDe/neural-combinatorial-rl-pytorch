...
val_fname = tsp_task.create_dataset(problem_size=str(size), data_dir=data_dir)
...
training_dataloader = DataLoader(training_dataset, batch_size=int(args['batch_size']), shuffle=True, num_workers=0)
...
for i in range(epoch, epoch + int(args['n_epochs'])):
    ...
    if args['is_train']:
        ...
        # sample_batch is [batch_size x input_dim x source_length]
