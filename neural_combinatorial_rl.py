    ...
    def forward(self, decoder_input, embedded_inputs, hidden, context):
        ...
        def recurrence(x, hidden, logit_mask, prev_idxs, step):
            ...
            for i in range(self.n_glimpses):
                ...
                # [batch_size x h_dim x source_length] * [batch_size x source_length x 1] =
...
class PointerNetwork(nn.Module):
    ...
    def forward(self, inputs):
        ...
        Args: 
            inputs: [source_length x batch_size x embedding_dim]
        """
...
class CriticNetwork(nn.Module):
    ...
    def forward(self, inputs):
        """
        Args:
            inputs: [embedding_dim x batch_size x source_length] of embedded inputs
        """
...
class NeuralCombOptRL(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # Result is [source_length x batch_size x embedding_dim]
        ...
        actions = []
        # inputs is [batch_size, input_dim, source_length]
        inputs_ = inputs.transpose(1, 2)
        # inputs_ is [batch_size, source_length, input_dim]
        for action_id in action_idxs:
            actions.append(inputs_[[x for x in range(batch_size)], action_id.data, :])

        if self.is_train:
            # probs_ is a list of len source_length of [batch_size x source_length]
            probs = []
            for prob, action_id in zip(probs_, action_idxs):
                probs.append(prob[[x for x in range(batch_size)], action_id.data])
        else:
            # return the list of len source_length of [batch_size x source_length]
            probs = probs_
